...
enqueue_buffered_messages(WPid, Horizon, RequestedTopics) ->
  ...
  BufferedMessages = lists:foldl(fun(RPid, MsgAcc) ->
  ...
  if Failure ->
    ...
    true ->
      lists:foreach(fun(MessagePack) ->
        {Message, Topics, _Timestamp} = MessagePack,
